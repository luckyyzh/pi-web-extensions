...
- **缓存友好**：不改 system prompt、不改写历史、不动态增删/重排工具、不自动 `/reload`；交接提醒仅 UI 展示。
...
```jsonc
{
  ...
  "handoffReminder": { "enabled": true }, // false = 关闭会话结束处的交接提醒
  "budgets": {
...
新增的明确保存纪律位于工具的静态 promptGuidelines 尾部，需用户手动 `/reload` 或新会话后生效；加载新版会改变一次请求前缀，可能使已有缓存失效一次。运行中不会动态改写提示、工具列表或历史。

交接纪律明确要求在实现完成并验证、提交/部署后、或遇到阻塞准备结束时保存 handoff。作为不依赖模型措辞的漏存兜底，扩展只要确认当前会话发生工作变更且该次 agent run 已结束，便显示一次 UI 提醒；没有工作变更的普通问答不提醒。提醒只调用 `ctx.ui.notify`，不追加消息、不 `triggerTurn`、不调用额外 LLM，也不读取助手结束文字做语义判断。每个 pending epoch 只提醒一次：未补存 handoff 前不会每轮重复骚扰。只有成功保存 `kind:"handoff"` 才清除本会话 pending 并重置提醒状态，之后再发生新变更才可能重新提醒；保存 knowledge、失败或取消的 handoff 均不清除。状态按扩展会话实例隔离，另一会话保存不会代替本会话交接。

变更跟踪采用保守边界：仅成功的 `write`、`edit`、`apply_patch`，以及通过 `bash` 单独执行且成功的 `git commit` 命令（排除 dry-run/help）算作确切变更。普通 shell 查询、复合 shell 命令、无法可靠识别的脚本、PowerShell、push、部署或子代理内部命令不算变更。可用 `handoffReminder.enabled:false` 关闭；缺失或非法配置回退为开启。
...
影子工作区检测、扩展冒烟（注册、git 子目录项目根、交接注入一次、退出兜底（有修改证据才写）、
